  ...
  const handleDelete = async (id: number, event: React.MouseEvent) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/shelf/delete/${id}`
      );

      if (response.status === 200) {
        const updatedData = data.filter((item: Shelf) => item.id !== id);
        setData(updatedData);
      }
    } catch (error) {
    ...
    <div className=" rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <form>
        <div className="flex flex-auto mb-9 mt-2 items-center">
          ...
          <NavLink
            to="/admin/shelf-management/create"
        ...
        <div className="max-w-full overflow-x-auto pb-5">
          {loading ? (
            ...
            <table className="w-full table-auto">
              ...
              <tbody>
                {currentItems.map((items: Shelf, id: number) => (
                  <tr key={id}>
