...
const StockTable = () => {
  ...
  const handlePageClick = (event: ReactPaginateProps) => {
  ...
  const handleDelete = async (id: number, event: React.MouseEvent) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/stock/delete/${id}`
      );

      if (response.status === 200) {
        const updatedData = data.filter((item: Stock) => item.id !== id);
        setData(updatedData);
      }
    } catch (error) {
    ...
    <div className=" rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <form>
        <div className="flex flex-auto mb-9 mt-2 items-center">
          ...
          <NavLink
            to="/admin/stock-management/create"
        ...
        <div className="max-w-full overflow-x-auto pb-5">
          {loading ? (
            ...
            <table className="w-full table-auto">
              ...
              <tbody>
                {currentItems.map((items: Stock, id: number) => (
                  <tr key={id}>
                    ...
                    <td className="min-w-[320px] border-b justify-center items-center border-[#eee] py-5 px-4 pl-5 dark:border-strokedark xl:px-8 xl:py-6 xl:pl-10">
                      <p className="text-black dark:text-white">
                        {items.books}
                      </p>
                    </td>
