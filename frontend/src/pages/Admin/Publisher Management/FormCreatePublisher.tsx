...
const FormCreatePublisher = () => {
  const navigate = useNavigate();
  const [error, setError] = useState<string | null>(null);
  const [inputValue, setInputValue] = useState<Publisher>({
    name: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:3000/api/publisher/create',
        inputValue
      );
      ...
      if (response.status === 201) {
        setError(null);
        setInputValue({ name: '' });
        return navigate('/admin/publisher-management');
      } else {
        setError('An error occurred while creating the publisher.');
      }
    } catch (error) {
      setError('An error occurred while creating the publisher.');
    }
  ...
  return (
    <>
      <div className="mx-auto max-w-270">
        <Breadcrumb pageName="Form Create Publisher" />
...
export default FormCreatePublisher;
