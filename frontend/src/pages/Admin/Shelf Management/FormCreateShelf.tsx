...
const FormCreateShelf = () => {
  const navigate = useNavigate();
  const [error, setError] = useState<string | null>(null);
  const [inputValue, setInputValue] = useState<Shelf>({
    name: '',
    description: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:3000/api/shelf/create',
        inputValue
      );
      ...
      if (response.status === 201) {
        ...
        return navigate('/admin/shelf-management');
      } else {
  ...
  return (
    <>
      <div className="mx-auto max-w-270">
        <Breadcrumb pageName="Form Create Shelf" />
...
export default FormCreateShelf;
